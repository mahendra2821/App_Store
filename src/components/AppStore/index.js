...
class AppStore extends Component {
  state = {acticveTabId: tabsList[0].tabId, searchInput: ''}

  setActiveTabId = tabId => {
    this.setState({acticveTabId: tabId})
  }
  onChangeSearchInput = event => {
    this.setState({searchInput: event.target.value})
  }
  getActiveTabId = searchedApps => {
    const {acticveTabId} = this.state
    const filterTabId = searchedApps.filter(
      eachSearchApp => eachSearchApp.category === acticveTabId,
    )
    return filterTabId
  }
  getSearchResults = () => {
  ...
  render() {
    const {searchInput, acticveTabId} = this.state
    const searchResults = this.getSearchResults()
    const filterTabId = this.getActiveTabId(searchResults)
          ...
          <ul className="tabs-List">
            {tabsList.map(eachTab => (
              <TabItem
                ...
                isActive={acticveTabId === eachTab.tabId}
              />
            ))}
          </ul>
          <ul className="apps-List">
            {filterTabId.map(eachApp => (
